%% define parameters and folders names
...
%% look at the first image to check the calibration settings
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
thresholdVal = 0.3; % parameters that you can change
minArea = 200; % parameters that you can change
maxArea = 700; % parameters that you can change
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%% perform regionprops
T = adaptthresh(imgaussfilt(A,1),thresholdVal);
...
title('filtered region props results')

%%%%%%
figure,
histogram([stats.Area],[0:10:10000])
title('surface of the found regions')
xlabel('area')

%%%%%%
for is = 1 : length(stats)
    astemp(is) = stats(is).BoundingBox(3)/stats(is).BoundingBox(4); % aspect ratio
end
figure,
histogram(astemp)
title('aspect ration of found regions')

hAS_Area = figure;
plot(astemp,[stats.Area],'sk')
% ylim([0 1000])
title('area VS aspect ration')
xlabel('aspect ratio')
ylabel('area')

clear iKill Xst Yst
iKill = find([stats.Area] < minArea);
stats(iKill) = [];
clear iKill
iKill = find([stats.Area] > maxArea);
stats(iKill) = [];
...
figure(hAS_Area), hold on
plot(astemp,[stats.Area],'sb','markerFaceColor','b')
...
%% do calibration on all calibration images
close all
clear listNames mirePoints
mirePoints = struct();
listNames = dir('*.tif');

% Determination of number of cameras and number of planes
...
for i = 1 : length(listNames)
    ...
    stats = regionprops(BW,'Centroid','Area','boundingbox','perimeter','convexHull');
    clear iKill Xst Yst
    iKill = find([stats.Area] < 200);
    stats(iKill) = [];
    clear iKill
    iKill = find([stats.Area] > 700);
    stats(iKill) = [];
    clear iKill
    iKill = [];
    for is = 1 : length(stats)
...
for is = 1 : length(stats)
    ...
    if (xCoordEps + yCoordEps < 0.25) && ...
            (xCoord>=xCmin && xCoord<=xCmax) && ...
            (yCoord>=yCmin && yCoord<=yCmax)
        figure(hBW), hold on
