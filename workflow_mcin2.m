...
%% TO KEEP  build data for quiver 3D

% U V and W are toped at 
Uborder = .4; % m/s
Vborder = .4; % m/s
Wborder = .4; % m/s
...
% prepare voxData that will contain speeds vx vy vz in voxels defines by
% listx listy listz
% the voxel size is wVox
clear voxData U V W
wVox = 1;
listX = -20 : wVox : 20;
listY = -22 : wVox : 12;
...
for ix = 1:length(listX)-1
    for iy = 1:length(listY)-1
        for iz = 1:length(listZ)-1
            ...
            U(ix,iy,iz) = max(-Uborder,min(Uborder,voxData(ix,iy,iz).U));
            V(ix,iy,iz) = max(-Vborder,min(Vborder,voxData(ix,iy,iz).V));
            W(ix,iy,iz) = max(-Wborder,min(Wborder,voxData(ix,iy,iz).W));
...
%%  TO KEEP display histogram of the found speeds in the three directions:
figure, hold on
title('speeds in x direction')
U2 = U;
U2(U2==0) = NaN;
histogram(U2(:))

figure, hold on
title('speeds in y direction')
V2 = V;
V2(V2==0) = NaN;
histogram(V2(:))

figure, hold on
title('speeds in z direction')
W2 = W;
W2(W2==0) = NaN;
histogram(W2(:))
%%  TO KEEP 3D quiver 
